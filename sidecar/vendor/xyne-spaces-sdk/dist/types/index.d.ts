 ...
 * `createdByUserId`.
 *
 * Participant counts and last-activity timestamps are NOT on this type: they are
 * maintained in a separate rollup and read via {@link ChannelsResource.getStats} or
 * {@link ChannelsResource.getStatsForChannels}. A channel with no counter row is
 * simply absent from those results — fall back to zero / the channel's `createdAt`.
 */
...
export type AttachmentEntityType = 'TICKET' | 'CHAT' | 'CANVAS' | 'DRAFT' | 'DELAYED_MESSAGE' | 'EMAIL' | 'IMPACT' | 'COLLECTION' | 'FORM_ENTITY_VALUE' | 'WORKFLOW_STEPS' | 'DESK_REPORT';
 ...
 */
export type UploadFileInput = Blob | {
    file: Blob;
    filename?: string;
    thumbnail?: Blob;
...
export type TicketActivityType = 'TITLE' | 'DESCRIPTION' | 'STATUS' | 'ASSIGNED_TO' | 'TICKET_TYPE' | 'PRIORITY' | 'ETA' | 'STAGE_ETA' | 'METADATA' | 'CLOSED_AT' | 'CLOSED_BY' | 'REFERENCE_TICKET' | 'STAGE_NAME' | 'TAGS' | 'ENTITY' | 'SUBTICKET_CREATED' | 'SUBTICKET_LINKED' | 'SUBTICKET_UNLINKED' | 'BOARD' | 'PR' | 'USER_GROUP_ID' | 'PR_REVIEWER' | 'QA' | 'STAGE_CHANGE_REQUEST' | 'STAGE_CHANGE_APPROVED' | 'STAGE_CHANGE_REJECTED' | 'IS_ARCHIVED' | 'MERGED' | 'UNMERGED' | 'RCA_CREATED' | 'RCA_UPDATED' | 'EMAIL_SENT' | 'TICKET_CREATED' | 'CSAT_RECEIVED';
/** How two linked tickets relate. */
export type TicketReferenceRelation = 'LINKED' | 'DUPLICATE_CONFIRMED' | 'DUPLICATE_POSSIBLE' | 'MERGED_INTO';
...
export type LookupType = 'TICKET_TYPE' | 'COE_ACTION_TYPE' | 'COE_ACTION_TYPE_RELIABILITY_CHANGE' | 'COE_ACTION_TYPE_RELIABILITY_CAPACITY' | 'COE_ACTION_TYPE_RELIABILITY_FAULT' | 'COE_ACTION_TYPE_PERF' | 'COE_ACTION_TYPE_UIUX' | 'IMPACT_TYPE' | 'BUG_TYPE' | 'BUG_CATEGORY_TYPE' | 'BUG_ISSUE_TYPE' | 'BUG_ISSUE_CATEGORY_CAPACITY' | 'BUG_ISSUE_CATEGORY_CHANGE' | 'BUG_ISSUE_CATEGORY_FAULT' | 'BUG_RESOLUTION_CAPACITY' | 'BUG_RESOLUTION_CHANGE' | 'BUG_RESOLUTION_FAULT' | 'QUICK_FIX_OPTION';
...
export type NudgeKind = 'CREATE_TICKET_FROM_MESSAGE' | 'FIND_RELATED_TICKET_FROM_MESSAGE' | 'FIND_RELATED_MESSAGE_FROM_MESSAGE' | 'LINK_PASTE_TO_SURFACE' | 'FORWARD_MESSAGE_LINK' | 'DELETE_MESSAGE_CLEANUP' | 'SCHEDULE_CALL_FROM_THREAD';
...
export interface Dashboard {
    ...
    queryMappings?: DashboardQueryMapping[];
}
 ...
 */
export type SearchSchemaName = 'chat_message' | 'chat_attachment' | 'chat_container' | 'ticket' | 'user' | 'file' | 'sam_transcript' | 'mail' | 'mail_attachment' | 'project' | 'memory' | 'call';
 ...
 */
export type SearchType = 'messages' | 'attachments' | 'calls' | 'channels' | 'tickets' | 'users' | 'files' | 'canvas' | 'transcript' | 'rca' | 'people' | 'emails';
/** Apps `SearchOptions.apps` accepts. Mirrors `APPS` in the contract. */
export type SearchApp = 'chat' | 'ticket' | 'user' | 'file' | 'collection' | 'mail' | 'xyneapp' | 'call';
/**
 * Search parameters.
 *
